...
where
    ...
    AND CC_NOMINA IN (
      'MT70', 'MT71', 'MT72', 'MT80', 'MT81', 'MT82',
      'MT74', '/TTK', 'MI11',
      '1001', '1002',
      '1007', '1008', '1010', '1011', '1012', '1013', '1014', '1015', '1016',
      '1017', '1018', '1019', '1029', '1031', '1032', '1033', '1034', '1035',
      '1036', '1042', '1060', '1065', '1066', '1067'
    )
),
...
select
	...
	case
		when 
			LENGTH(MC.FECHA_ANTIGUEDAD) = 12 
				then cast(TO_TIMESTAMP(SUBSTRING(MC.FECHA_ANTIGUEDAD, 1, 9)::BIGINT) + interval '1 DAYS' as DATE)
		else cast(TO_TIMESTAMP(SUBSTRING(MC.FECHA_ANTIGUEDAD, 1, 10)::BIGINT) + interval '1 DAYS' as DATE)
	end as FECHA_INGRESO,
	case
		when 
			MC.FECHA_FIN_CONTRATO = '32503680000000' 
			then cast(TO_TIMESTAMP(SUBSTRING(MC.FECHA_FIN_CONTRATO, 1, 11)::BIGINT) + interval '1 DAYS' as DATE)
	...
	case
		when 
			%(fecha_desvinculacion)s::DATE - (
				case
					when LENGTH(MC.FECHA_ANTIGUEDAD) = 12 
						then cast(TO_TIMESTAMP(SUBSTRING(MC.FECHA_ANTIGUEDAD, 1, 9)::BIGINT) + interval '1 DAYS' as DATE)
					else cast(TO_TIMESTAMP(SUBSTRING(MC.FECHA_ANTIGUEDAD, 1, 10)::BIGINT) + interval '1 DAYS' as DATE)
				end
		) 
